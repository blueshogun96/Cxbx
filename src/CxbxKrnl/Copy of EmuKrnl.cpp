...
typedef struct _CriticalSectionTracker
{
	DWORD* XboxAddress;
	NtDll::RTL_CRITICAL_SECTION cs;
...
CriticalSectionTracker CSTracker[64];
int g_iCSCount = 0;
...
)
{
	...
	{
		EmuSwapFS();
		xboxkrnl::RtlEnterCriticalSection(CriticalSection);
		EmuSwapFS();
	}

	while( i < 63 )
	{
		if( CSTracker[i].XboxAddress == (DWORD*) CriticalSection )
		{
			NtDll::RtlEnterCriticalSection((NtDll::_RTL_CRITICAL_SECTION*)&CSTracker[i].cs);
			break;
		}
	}
...
)
{
	...
	// How many critical sections do we have now?
	if( g_iCSCount > 63 )
		CxbxKrnlCleanup( "Too many critical sections (easy fix, tell blueshogun)" );

//    NtDll::RtlInitializeCriticalSection((NtDll::_RTL_CRITICAL_SECTION*)CriticalSection);

	// Initialize the PC critical section
	NtDll::RtlInitializeCriticalSection((NtDll::_RTL_CRITICAL_SECTION*)&CSTracker[g_iCSCount].cs);

	// Save the address of the Xbox critical section
	CSTracker[g_iCSCount].XboxAddress = (DWORD*) CriticalSection;

	// Fill in the appropriate fields
	CriticalSection->LockCount = CSTracker[g_iCSCount].cs.LockCount;
	CriticalSection->RecursionCount = CSTracker[g_iCSCount].cs.RecursionCount;
	CriticalSection->OwningThread = CSTracker[g_iCSCount].cs.OwningThread;

	// Increase the critical section count
	g_iCSCount++;
...
//    NtDll::RtlLeaveCriticalSection((NtDll::_RTL_CRITICAL_SECTION*)CriticalSection);
	...
	while( i < 63 )
	{
		if( CSTracker[i].XboxAddress == (DWORD*) CriticalSection )
		{
			NtDll::RtlLeaveCriticalSection((NtDll::_RTL_CRITICAL_SECTION*)&CSTracker[i].cs);
			break;
		}
	}
...
)
{
	...
	while( i < 63 )
	{
		if( CSTracker[i].XboxAddress == (DWORD*) CriticalSection )
		{
			bRet = NtDll::RtlTryEnterCriticalSection((NtDll::_RTL_CRITICAL_SECTION*)&CSTracker[i].cs);
			break;
		}
	}
